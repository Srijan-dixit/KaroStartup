...
voice_left = mixer.Sound('C:/Users/Asus/Desktop/Voice/left.wav')
voice_right = mixer.Sound('C:/Users/Asus/Desktop/Voice/Right.wav')
voice_center = mixer.Sound('C:/Users/Asus/Desktop/Voice/center.wav')
...
def positionEstimator(cropped_eye):
    ...
    cv.imshow('gb',cv.resize(gaussain_blur,(800,500)))
    median_blur = cv.medianBlur(gaussain_blur, 3)
    cv.imshow('mb',cv.resize(median_blur,(800,500)))
    kernel = np.array([[-1,-1,-1], 
                       [-1, 9,-1],
                       [-1,-1,-1]])
    sharpened = cv.filter2D(median_blur, -1, kernel)
    cv.imshow('sharp',cv.resize(sharpened,(800,500)))
    at = cv.adaptiveThreshold(sharpened,255,cv.ADAPTIVE_THRESH_MEAN_C,\
            cv.THRESH_BINARY,11,2)
    cv.imshow('adaptive',cv.resize(at,(400,250)))
    
        
    # applying thrsholding to convert binary_image
    ret, threshed_eye = cv.threshold(median_blur, 130, 255, cv.THRESH_BINARY)
    cv.imshow('thresh_eye',threshed_eye)

    cv.imshow('cropped_eye',cv.resize(cropped_eye,(800,500)))
    ...
    #sr
    cv.imshow('right',cv.resize(right_piece, (960, 540)))
    cv.imshow('center',cv.resize(center_piece, (960, 540)))
    cv.imshow('left',cv.resize(left_piece, (960, 540)))
